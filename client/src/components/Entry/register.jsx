...
import Login from "./logIn.jsx"; // שימי לב למיקום הנכון
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!userName || !email || password !== verifyPassword) {
      setError("אנא מלא/י את כל השדות כראוי");
      return;
    }
    ...
    try {
      await newUser("users/new", { userName, email, password });
      const user = getCurrentUser();
      if (user) closeModal(); // סוגר את המודאל אחרי הרשמה
      else setError("שגיאה באימות המשתמש לאחר הרישום");
    } catch (err) {
  ...
  return (
    <div className="register-container fade-in">
      ...
      <form onSubmit={handleSubmit} className="register-form">
        ...
        <button type="submit" className="submit-button">רישום</button>
      </form>
      <button
